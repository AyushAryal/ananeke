...
def _execute(filters=None, additional_table_columns=None, additional_conditions=None):
    ...
    for d in item_list:
        ...
        if filters.get("group_by"):
            ...
            add_sub_total_row(
                row, total_row_map, d.get(group_by_field, ""), tax_columns
            )
    ...
    if filters.get("warehouse"):
        if frappe.db.get_value("Warehouse", filters.get("warehouse"), "is_group"):
            lft, rgt = frappe.db.get_all(
                "Warehouse",
                filters={"name": filters.get("warehouse")},
                fields=["lft", "rgt"],
                as_list=True,
            )[0]
            warehouses = frappe.db.get_all(
                "Warehouse", {"lft": (">", lft), "rgt": ("<", rgt)}, pluck="name"
            )
                                ...
                                frappe._dict(
                                    {
                                        ...
                                        "is_other_charges": (
                                            0
                                            if tuple([account_head]) in tax_accounts
                                            else 1
                                        ),
                                    }
                                )
...
def add_total_row(
    ...
    if prev_group_by_value != item.get(group_by_field, ""):
        ...
        total_row_map.setdefault(
            ...
            {
                subtotal_display_field: get_display_value(
                    filters, group_by_field, item
                ),
                "stock_qty": 0.0,
                "amount": 0.0,
                "bold": 1,
...
def add_sub_total_row(item, total_row_map, group_by_value, tax_columns):
    ...
    for tax in tax_columns:
        ...
        total_row[frappe.scrub(tax + " Amount")] += flt(
            item[frappe.scrub(tax + " Amount")]
        )
